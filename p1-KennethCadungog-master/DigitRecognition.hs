...
helperPixelImage row = [if x == True then '#' else ' ' | x <- row]
showPixelImage img = unlines [helperPixelImage y | y <- img]
...
buildCorpus imgLbls = [(x, [ fst img | img <- imgLbls, snd img == x]) | x <- nub [snd img | img <- imgLbls]] -- [x <- [0...9] | (x, [img | x == dig, (dig, img) <- imgLbls])] 
--
--                                  Core Project 
...
probOfDigit corpus digit = outOf (sum([length(snd c) | c <- corpus, fst c == digit])) (sum[length(snd c) | c <- corpus])
...
probOfFeature imgs ftr = outOf (1 + (sum[ 1 | i <- imgs, hasFeature i ftr == True])) (2 + (sum[1 | i <- imgs]))
...
probOfNoFeature imgs ftr = 1 - probOfFeature imgs ftr 
...
rankOfDigit corpus digit newImg = probOfDigit corpus digit * product [if hasFeature newImg n then probOfFeature a n else probOfNoFeature a n | n <- allFeatures]
    where a = lookupVal digit corpus
...
classifyImage corpus newImg = snd (maximum [(rankOfDigit corpus dig newImg, dig) | dig <- allDigits])
